...
for i=1:gen
    ...
    for j=1:pop_size/4
        index=randi(o);
        %JOB-BASED CROSSOVER
        offspring = crossoverjobbased(Solutions(p(2*j-1),:),Solutions(p(2*j),:),n,m);
    ...
    for j=1:pop_size/2
        ...
        %MUTATION FOR JOB BASED REPRESENTATION
        offspring_mut = mutationjobbased(Solutions(K(j),:),n,m);
